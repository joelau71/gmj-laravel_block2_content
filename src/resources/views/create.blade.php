<x-admin.layout.app>
    ...
    <form id="myform" method="POST"
        action="{{ route('LaravelBlock2Content.create', $element_id) }}" class="relative" enctype="multipart/form-data">
        ...
        @foreach (config('translatable.locales') as $locale)
            <x-admin.atoms.row>
                ...
                <textarea name="text[{{ $locale }}]" id="text_{{ $locale }}" class="tinymce-textarea bg-white" style="height: 200px;"></textarea>
                @error("text.*")
                    <x-admin.atoms.error>
                        {{ $message }}
                    </x-admin.atoms.error>
